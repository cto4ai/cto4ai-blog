...
contentType: 'episode'
description: "My thoughts on Kevin Hou's talk about Antigravity, DeepMind's new agent-first development platform."
author: 'Jack Ivers'
...
Google DeepMind has definitely gotten my attention with significant releases over the past few weeks, in particular [Gemini 3 Pro](https://blog.google/products/gemini/gemini-3/), [Gemini 3 Pro Image](https://deepmind.google/models/gemini-image/pro/) aka [Nano Banana Pro](https://chromeunboxed.com/the-hilarious-230-am-origin-story-of-how-nano-banana-got-its-name/), and most recently [Antigravity](https://developers.googleblog.com/build-with-google-antigravity-our-new-agentic-development-platform/). Based on [this talk](https://www.youtube.com/watch?v=xmbSQz-PNMM&t=29388s) by [Kevin Hou](https://www.linkedin.com/in/kevinhou22/) at the recent [AI Engineer Code Summit](https://www.ai.engineer/code), as well as the [ThursdAI](https://thursdai.news/) podcast's chat with Hou recorded at the same event, I've made it a priority to get hands-on with Antigravity, and find it to be a new and different take on the coding-with-AI experience. If, with [Claude Code](https://www.anthropic.com/claude-code), Anthropic's message was:
...
Two of these components are, effectively, "new things" in an AI Coding tool. Other platforms can "do browser stuff" and might have some agent/subagent action happening, but nobody[^1] delivers browser and agent manager elements that approach the level of power and integration that I'm seeing in Antigravity.
...
But secondly, to get my head around why Hou was claiming that Nano Banana Pro was a big win for _developers_. I've spent a lot of time using NBP, and repeatedly amazed at what it was creating for me. But I hadn't made the connection between NBP's incredible image generating power and the UI design process that is a critical element of virtually all application development work. It's the design, silly!
...
I make no endorsement, express or implied, of the model's self-described "Dark Glassmorphism" theme, but still, this is really impressive app theme, almost viable as a first design iteration, _from a 14-word prompt_ that provided no stylistic guidance. [FIXME-this is a meta prompt] Note that Hou's example prompt has a more realistic level of detail:

```
A high-fidelity, modern website design for a plant shop or showcase in light mode.
The design should be clean, airy, and elegant, using a color palette of soft whites,
creams, and various shades of lush green. The hero section should feature a
stunning, high-resolution image of a beautiful indoor plant (like a Monstera or Fiddle
Leaf Fig) in a stylish pot, with bold but refined typography for the headline 'Bring
Nature Home'. Below the hero, a grid layout showcasing a variety of plants
(succulents, ferns, hanging plants) with minimalist cards containing plant names
and prices. The Ul should be sophisticated with plenty of whitespace, subtle
shadows, and a sticky navigation bar at the top with links like 'Shop', 'Care Guide',
'About'. The overall vibe should be fresh, organic, and premium.
```

And the output is stunningly good:

And cool gets even cooler---note that I selected a region of the image and added a comment. Hou:
...
Those comments feed right back to the Agent Manager, which is then _fully capable of building what Nano Banana designs_. The resulting application would almost certainly have a much better design, and be far more robust and maintainable, than anything a prompt-to-app tool like Bolt could produce.
