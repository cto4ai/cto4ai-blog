...
- That contentType property would enable special behaviors when content is rendered (such as we have with Elsewhere content and in our placeholder images in content lists)
- Both content mdx and md files, and their associated images, would be stored in a type-independent folder structure
  - mdx and md content would be stored under a single /src/data/xxx/ directory  where xxx is a content-type-independent name; possibly repurposing the generic "posts" name. Each post would have a unique {postDir}. Example then might be: /src/data/posts/this-is-a-post/
